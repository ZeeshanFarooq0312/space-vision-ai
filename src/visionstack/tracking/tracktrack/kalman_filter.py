"""Constant-velocity Kalman filter over (cx, cy, w, h). Vendored verbatim from TrackTrack
(kamkyu94/TrackTrack, trackers/kalman_filter.py, MIT license) -- no behavioral changes.
"""
...
class KalmanFilter:
    ...
    def project(
        self, mean: np.ndarray, covariance: np.ndarray, confidence: float
    ...
    def update(
        self, mean: np.ndarray, covariance: np.ndarray, measurement: np.ndarray, confidence: float
    ) -> tuple[np.ndarray, np.ndarray]:
        ...
        kalman_gain = scipy.linalg.cho_solve(
            (chol_factor, lower), np.dot(covariance, self.update_mat.T).T, check_finite=False
        ).T
